...
//
// Every named effect below EXCEPT playStoneStrike() (which has its own
...
// separate short noise-tick voice for buttons. The old tone()-based
// win/lose/draw
...
// The whole scale is written out even where a degree is not currently
// voiced (B3, FS4): the point of the table is the SCALE, and a reader who
// finds four notes cannot tell which five-note set they belong to.
const A3 = 220;
const B3 = 246.9;
const CS4 = 277.2;
const E4 = 329.6;
 ...
 *   isDailyChallenge flag isn't threaded through to this call yet (this
 ...
 *   disclosed-but-unwired parameter, kept because the campaign's final
 *   stage is exactly the moment it is for.
 */
export function playWinSound(isDaily = false) {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  if (voiceFromSample("win", 1)) return;
  ...
  // allowed to reach playStoneStrike()'s own peak (0.17) — CLAUDE.md's
  // own "게인 스테이징" rule.
...
export function playButtonSound() {
  if (voiceFromSample("button", 1, 0.7)) return;
...
/** A stone hitting a peg or a wall: the same strike, darker and deader,
 * because the thing being hit is fixed and absorbs rather than rings.
 *
 * Kept for the obstacles StoneFlick had and this game does not. The
 * cushion used to borrow it and no longer does — see playCushionSound().
 */
export function playObstacleSound(strength01 = 1) {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  const strength = impactStrength(strength01);
  if (strength < 0.08) return;
  if (voiceFromSample("obstacle", strength)) return;
  makeMallet(audioCtx, audioCtx.destination, CS4 * (0.95 + Math.random() * 0.1), audioCtx.currentTime, 0.04 + strength * 0.09, 0.07 + strength * 0.06, {
    lowpassFreq: 1900 + strength * 900,
    wetRatio: 0.08,
  });
}
...
/**
 * A stone off a brass bumper. Deliberately NOT the obstacle sound with
 * different numbers: the bumper is the only metal on the board and the
 * only thing that gives energy back, and both facts should be audible
 * before the stone has finished leaving. So where the peg's voice is
 * dark, short and dead, this one is bright, rings on, and pitches UP with
 * force — the same reasoning the renderer uses for making it the only
 * metal object, applied to sound.
 */
export function playBumperSound(strength01 = 1) {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  const strength = impactStrength(strength01);
  if (strength < 0.05) return;
  if (voiceFromSample("bumper", strength)) return;
  const startAt = audioCtx.currentTime;
  const base = E5 * (0.97 + Math.random() * 0.06) * (1 + strength * 0.18);
  const options = { lowpassFreq: 6000 + strength * 4000, wetRatio: 0.3 };
  // Two partials a fifth apart, the second quieter and longer: a struck
  // metal object rings on a small number of clear tones, where wood
  // thuds on none.
  makeMallet(audioCtx, audioCtx.destination, base, startAt, 0.06 + strength * 0.1, 0.3 + strength * 0.45, options);
  makeMallet(audioCtx, audioCtx.destination, base * 1.5, startAt + 0.004, 0.03 + strength * 0.05, 0.45 + strength * 0.5, options);
}

/**
 * A stone going through a portal. The quietest event that gets a sound at
 * all, and it has to be: teleporting is startling enough on its own, and
 * the stone is still travelling when it happens — anything with a
 * transient would be mistaken for a collision that did not occur.
 */
export function playPortalSound() {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  if (voiceFromSample("portal", 1)) return;
  const startAt = audioCtx.currentTime;
  // Rising, soft, very wet: up and away, in a bigger room than the board.
  const options = { lowpassFreq: 2600, wetRatio: 0.55 };
  makeMallet(audioCtx, audioCtx.destination, A4, startAt, 0.05, 0.28, options);
  makeMallet(audioCtx, audioCtx.destination, CS5, startAt + 0.05, 0.045, 0.4, options);
  makeMallet(audioCtx, audioCtx.destination, E5, startAt + 0.1, 0.04, 0.55, options);
}
...
/** A stone going over the edge. A short falling pair — the same "down"
 * gesture as playLoseSound() but far smaller, since losing a stone is a
 * turn's outcome, not the game's. */
export function playFallSound() {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  if (voiceFromSample("fall", 1)) return;
  const startAt = audioCtx.currentTime;
  const options = { lowpassFreq: 2600, wetRatio: 0.18 };
  makeMallet(audioCtx, audioCtx.destination, E4, startAt, 0.09, 0.22, options);
  makeMallet(audioCtx, audioCtx.destination, A3, startAt + 0.09, 0.08, 0.4, options);
}

/**
 * Two or more of the opponent's stones off the board on one shot — what
 * Korean alkkagi players call 일타이피, "one strike, two kills."
 *
 * Rises further the more it took out, and that is the whole design: the
 * player already SAW the stones go, so this sound is not information, it
 * is the reward. Making a triple sound like a double with an extra note
 * would be the flat version of the same idea; each step also starts
 * brighter and rings longer.
 * @param {number} count - opponent stones removed by this one shot, >= 2
 */
export function playComboSound(count = 2) {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  const steps = Math.min(count, 4);
  const startAt = audioCtx.currentTime;
  const scale = [A3, E4, A4, CS5, E5];
  for (let i = 0; i < steps; i++) {
    makeMallet(audioCtx, audioCtx.destination, scale[i + 1], startAt + i * 0.075, 0.1 + i * 0.015, 0.2 + i * 0.12, {
      lowpassFreq: 4200 + i * 900,
      wetRatio: 0.22,
    });
  }
}

/** A stone dropping into a hole. Same falling shape as playFallSound()
 * but lower, longer and much wetter — the extra reverb send is the whole
 * point, since "it went DOWN into something" is a depth cue and depth is
 * what reverb encodes. */
export function playSinkSound() {
  if (!shouldPlaySound()) return;
  const audioCtx = getContext();
  if (!audioCtx) return;
  if (voiceFromSample("sink", 1)) return;
  const startAt = audioCtx.currentTime;
  const options = { lowpassFreq: 1500, wetRatio: 0.42 };
  makeMallet(audioCtx, audioCtx.destination, CS4, startAt, 0.1, 0.3, options);
  makeMallet(audioCtx, audioCtx.destination, A3 / 2, startAt + 0.1, 0.09, 0.75, options);
}
