...
test("i18n: every block of the .content article is keyed, and nothing in it is left as unkeyed prose", () => {
  ...
  assertTrue(/data-i18n-module="\/solitaire\/src\/i18n\/content\.js"/.test(article),
    "the article declares /solitaire/src/i18n/content.js");
  ...
  assertTrue(body.includes('<h1 class="game-title" data-i18n-content="game.solitaire.name">Solitaire</h1>'), "the h1 is keyed to the shared game name");
...
test("i18n: src/i18n/content.js holds no English, and every language in it is complete", async () => {
  ...
  for (const [code, dict] of Object.entries(content)) {
    ...
    // All or nothing: a page half in Solitaire's language and half in English
