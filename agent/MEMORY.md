...
- pulse.mjs는 CSV에 이미 있는 날짜 행을 갱신하지 않는다(tools/agent/pulse.mjs `have.has(d)`). 06:30 로컬 실행이라 '어제' 행은 GA4 부분집계로 고정될 수 있고, 09-16/09-17 행의 GSC 열은 빈칸으로 남는다 (펄스, 2026-09-18)
