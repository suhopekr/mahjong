...
3. **재방문 훅** (PWA 설치 안내, 이어서 하기)의 효과를 데이터로 확인한 뒤 확장. Paid Social 기간 이후 재방문이 거의 0인 것이 확인됐다.
   3b. (대기) **UX 관찰 트랙.** 오가닉이 하루 30 세션 이상 2주 연속이면 행동 이벤트(설정 열기, 타일 크기 변경, 힌트/되돌리기 사용, 포기 시점, PWA 설치)를 제안하고, 펄스가 그것을 읽게 한 뒤 UI 변경을 **제안**한다. UI 자체는 항상 창조자가 결정한다 — 디자인이 이 사이트의 최우선이다. 그 전에는 UI 결론을 내리지 않는다(표본 부족).
