// game/physics.js
// Billiards physics: four spheres rolling on cloth inside a
// pocketless rectangle, with spin.
//
 ...
 * feel livelier. */
export const BALL_RADIUS = 0.03275;
export const BALL_MASS = 0.254;
...
export function stepWorld(world, dt) {
  ...
  while (remaining > 1e-12) {
    ...
    for (const b of world.balls) applyCloth(b, h, world);
...
function clampPositions(world) {
  for (const b of world.balls) {
    if (b.x < R) b.x = R;
    else if (b.x > world.length - R) b.x = world.length - R;
    if (b.y < R) b.y = R;
    else if (b.y > world.width - R) b.y = world.width - R;
  }
  const balls = world.balls;
  for (let i = 0; i < balls.length; i++) {
    for (let j = i + 1; j < balls.length; j++) {
